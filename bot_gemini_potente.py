...
async gemini_ai(mensaje: str) -> str:
    try:
        url = f"https://generativelanguage.googleapis.com/v1/models/gemini-pro:generateContent?key={GEMINI_KEY}"
        payload = {
            "contents": [{"parts": [{"text": f"Responde en español de forma útil y detallada: {mensaje}"}]}],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🧠 ¡Bot AI Potente activado! Escribe cualquier pregunta...")

async def chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = await update.message.reply_text("⚡ Consultando con Gemini AI...")
